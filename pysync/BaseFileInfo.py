...
def get_pending(diff_infos):
    ...
    # * important to sort by depth first, not just length of .path
...
def commit_drive_ops(diff_infos, all_data, drive):
    ...
    Applies the changes to folders first, then least depth
    ...
    def callback(fut):  # * btw surely there's a better way to define this function while getting access to these variables
            ...
            if info.op_success:
                ...
                for child_path in all_data:
                    ...
                    if contains_parent(info.path, child_path, accept_self=False):
                        child.failed_reason = "One of its parent folders failed"
                ...
                # * extend instead of adding it in the loop cos memory access like this causing delay i think
                failed.extend(temp_fail)
                failed.append(info)

    max_threads = get_option("MAX_UPLOAD")
    pending = get_pending(diff_infos)
    ...
    # TODO since there's logic associated with the order of submits
    # * must be processpool, threadpool runs into memory issue
    with cf.ProcessPoolExecutor(max_workers=max_threads,) as executor:

        while pending:
            # * Using index to be able to modify the list mid-loop im sure there's a cleaner way to do this
            index = len(pending) - 1
